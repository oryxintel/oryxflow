import oryxflow.tasks as tasks
import oryxflow
...
class Workflow:
    ...
    def task(self, task_type: oryxflow.tasks.TaskData):
        """
            Flow step decorator.
        ...
        """
        assert task_type.__base__ == tasks.TaskData, "Invalid parameter. Parameter should be type defined in oryxflow.tasks"
    ...
    def requires(self, *args, **kwargs):
        """
            ...
            Internally calls oryxflow.requires.
        ...
        self.steps[self.current_step] = oryxflow.requires(
    ...
    def params(self, **params):
        """
            ...
            Parameters
            ----------
            **params : keyword arguments of oryxflow parameters
    ...
    def preview(self, func_to_preview, params: dict):
        ...
        if func_params:
            oryxflow.preview(self.steps[name](**func_params))
        elif all_params:
            for params in self.params_used[name]:
                oryxflow.preview(self.steps[name](**params))
        else:
            oryxflow.preview(self.steps[name]())

    
    def run(self, funcs_to_run, params: dict = None, multi_params: dict = None, *args, **kwargs):
        """
            Runs flow steps locally.
            Parameters
            ----------
    ...
    def outputLoadAll(self, func_to_run, *args, **kwargs):
        ...
        if self.multi_params:
            ...
            for params in self.multi_params:
                print(self.multi_params_tasks[params])
                output[params] = {}
                tasks = oryxflow.taskflow_upstream(
                    self.multi_params_tasks[params])
                for task in tasks:
