---
date: 2026-07-10
slug: why-machine-learning-code-is-bad
categories:
  - Machine learning
...
class TaskProcess(oryxflow.tasks.TaskPqPandas): # define output format
...
Below is a stylized example of a machine learning flow which is expressed as a DAG. In the end you just need to run <span class="title-ref">TaskTrain()</span> and it will automatically know which dependencies to run. For a full example see
...
class TaskGetData(oryxflow.tasks.TaskPqPandas):  # save dataframe as parquet
...
class TaskPreprocess(oryxflow.tasks.TaskCachePandas):  # save data in memory
    ...
    def run(self):
        ...
        df_train = self.inputLoad() # quickly load required data
