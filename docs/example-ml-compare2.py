...
class ModelFeatures(oryxflow.tasks.TaskPqPandas):
    interaction = oryxflow.BoolParameter(default=False)
...
@oryxflow.requires(ModelFeatures)
class DataTrain(oryxflow.tasks.TaskPqPandas):
    persists = ['x','y']
...
@oryxflow.requires(DataTrain)
class ModelTrainOLS(oryxflow.tasks.TaskPickle):
...
@oryxflow.requires(DataTrain)
class ModelTrainLGBM(oryxflow.tasks.TaskPickle):
    max_depth = oryxflow.IntParameter(default=2)
    learning_rate = oryxflow.FloatParameter(default=0.1)
...
@oryxflow.requires({'data':ModelFeatures, 'data-train':DataTrain, 'ols':ModelTrainOLS, 'lgbm':ModelTrainLGBM})
class ModelEvalAll(oryxflow.tasks.TaskPqPandas):
...
params = dict()
flow = oryxflow.Workflow(ModelEvalAll, params)
flow.preview()#forced_all=True,confirm=False, forced_all_upstream=True)
flow.run()

# multi model comparison
df_train = flow.outputLoad()
...
df_trainX, df_trainY = flow.outputLoad(task=DataTrain)
