...
oryxflow turns a data-science script into a pipeline of **tasks**. You declare each step as a
task — what it *depends on* and what it *produces* — and the engine runs them in the right order,
skips anything already computed, and lets you load any result by name. No manual file paths, no
re-running the slow steps to test a fast one.

This page gets you from nothing to a running pipeline. For installation, follow the
`GitHub instructions <https://github.com/oryxintel/oryxflow#installation>`_ (``pip install oryxflow``).
...
* it inherits from a task type that decides the output format (parquet, pickle, in-memory, …), so
  you never write save/load code;
...
Parameters are how you try different settings without renaming files by hand. Give a task a
parameter and oryxflow caches a **separate output per value**, so you can switch between them
instantly, and changing one reruns exactly the tasks that depend on it.
...
:ref:`Automatic code invalidation <code-versioning>`.)
...
.. tip::
   ...
   The fastest way to a fully-structured project is the Claude Code plugin: it sets up the
   project layout and wires tasks for you. See
   :doc:`Using oryxflow with Claude Code <claude-plugin>`.

Next steps
------------------------------------------------------------

* :doc:`Transition to oryxflow <transition>` — turn an existing script into tasks.
* :doc:`Writing and Managing Tasks <tasks>` — dependencies, inputs, outputs, and save formats.
* :doc:`Advanced: Parameters <advparam>` — parameter inheritance and how it drives reruns.
* :doc:`Running Workflows <run>` — previewing, running, and resetting tasks.
* :ref:`Managing Complex Workflows <managing-complex-workflows>` — caching expensive granular
  work and resetting it selectively as you iterate.

An interactive version of the ML example is on
`mybinder <http://tiny.cc/d6tflow-start-interactive>`_, and a real-life project template is at
