...
takes a `oryxflow.tasks.TaskName` as parameter
...
If you have worked with oryxflow.task before you would remember having a `self` parameter passed to `run()` method.
...
All functional tasks are run as `oryxflow.task` under the hood.

So we require to run them as you would run any `oryxflow.task`
...
oryxflow also allows you to pass in parameters to these functions dynamically using `@flow.params()`
...
Some of the functions that are in oryxflow are available in the `Workflow()` object too!
