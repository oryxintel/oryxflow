...
With oryxflow you can Export and Import Tasks from other projects and files.
...
You can import tasks from other folder and files into the file you want to run the flow in.

.. code-block:: python

    scraper = oryxflow.FlowImport(path='../another-project/', module='tasks_export.py', path_data='data/')
    flow_import = oryxflow.Workflow(scraper.tasks.TaskScrape, path=scraper.dirpath, env='prod')
    flow_import.run()

FlowImport returns an object which contains all the Tasks imported from the file under `tasks` and dirpath which is the data directory location.
