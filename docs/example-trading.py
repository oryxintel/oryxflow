#************************************************************
# example trading strategy with oryxflow
# oryxflow is a framework for rapid prototyping and experiment tracking
...
# get economic data
class GetDataEcon(oryxflow.tasks.TaskPqPandas):
    date_start = oryxflow.DateParameter() # define parameters
    date_end = oryxflow.DateParameter() # define parameters
...
@oryxflow.requires(GetDataEcon) # define dependency
class TradingSignals(oryxflow.tasks.TaskPqPandas):
    lookback_period = oryxflow.IntParameter()
...
# get stock prices
@oryxflow.requires(GetDataEcon)
class GetDataPx(oryxflow.tasks.TaskPqPandas):
    symbols = oryxflow.ListParameter()
...
# run backtest
@oryxflow.requires(TradingSignals,GetDataPx)
class Backtest(oryxflow.tasks.TaskPqPandas):
...
import shutil
shutil.rmtree(oryxflow.settings.dirpath, ignore_errors=True)
...
for istrat, strategy in enumerate([strategy1,strategy2,strategy3]):
    ...
    flow = oryxflow.Workflow(Backtest, strategy)
    print(flow.preview())  # show which tasks will be run
    flow.run()
    df_pnl1 = flow.outputLoad(keys='pnl') # load task output
    print(f'pnl strategy #{istrat+1}:', df_pnl1.sum().sum().round(3))

def dev():
    oryxflow.Workflow(TradingSignals, strategy1).reset() # reset after making updates
