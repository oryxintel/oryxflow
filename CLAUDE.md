...
**Fan-out (one dep per value)** — `@requires_each(task, **grid)` is the fan-out sibling of `@requires`: copies the dep's params **minus** the fanned-out ones (the combining task must not carry them) and declares one dep per value. Keys: the value itself for one param, `name_value` joined by `_` for several. A grid value may be a **callable** taking the task, resolved at `requires()` time against its **params** (not its inputs — `input()` is `getpaths(self.requires())`, so reading inputs there recurses forever). `self.requires_grid(cls, **grid)` (core.py, next to `clone`) is the same thing callable from a hand-written `requires()`; several keywords give the cartesian product. `derive={'name': fn}` (both forms) sets a **further param per branch** from that branch's fanned values — `derive={'source': lambda v: SOURCE[v['region']]}`; the callable takes the values **dict** (never `self`, never positional — so adding a second fanned param doesn't break every callable) and must be deterministic in them, because the result lands in the branch's params and therefore its `task_id`. That is the point: it makes a per-branch lookup *invalidating*, where the same lookup inside `run()` is invisible to the cache. Derived names are **out** of the dep key (the key stays the fanned value) and **off** the combining task. `_check_grid` (core.py) validates the whole spec against the dep class at *class definition* for `@requires_each` (the dep is fully built by then) and at `requires()` time for the method form: every **fanned** and every **derived** name must be a Parameter of the dep class, because `clone()` builds its kwargs from `cls.get_params()` and silently **drops** the rest — a fanned name it has no param for used to give N keys pointing at one identical task (N copies of one output, tagged as if they were branches), and a derived one used to leave the branch on its default.

**Reserved param names** (`Register.RESERVED_PARAM_NAMES`, core.py) — `path`, `flows`, `cls`, `derive`. Declaring a Parameter with any of these raises `ValueError` at class definition. `path`/`flows` the engine sets on instances itself; `cls`/`derive` are `clone()`/`requires_grid()` arguments, so the argument shadows the param. All four fail silently otherwise, hence rejecting the name where it is written rather than at the far-away call that breaks. **Never write `{v: cls(param=v, shared=self.shared) for v in values}` in docs or examples** — that form only carries the params someone remembered to list, which is the bug class these exist to remove. (They replaced `utils.requires_grid()`, removed in the same change — a free function has no `self` to copy params from.)

**Decorators stack** (`@requires` / `@inherits` / `@requires_each`, any order, any number) — the normal combining task needs the fan-out **and** a shared un-fanned dep (the source table, a baseline, labels). Each decorator appends to a per-class `_requires_spec` (core.py) which `_apply_spec` re-resolves from scratch, so order is irrelevant; `_resolve_requires` generates the single `requires()`. The fanned-param exclusion is **global across the whole spec**, not per-decorator — otherwise `@requires(X)` would copy back an `X.region` that `@requires_each` deliberately excluded, silently. `derive=` names join the fanned ones in that same global exclusion set (`_apply_spec`'s `excluded`), for the same reason. Two things that used to be silently wrong now **raise**: declaring a fanned param in the combining task's body (`TypeError` — it would put one branch's value in the combiner's `task_id`), and duplicate dep keys (`ValueError`). Naming a group (`@requires_each({'chart': Chart}, region=...)`) qualifies its dep keys with that name (`chart_north`) — that is how a collision is resolved; unnamed groups keep bare value keys, so single-decorator shapes are unchanged. `_check_requires_not_overwritten` only fires for a genuinely hand-written `requires()` (generated ones carry `_oryxflow_generated`).
...
**Dependency queries** — `flow.dependents(X)` is the reverse lookup ("what depends on X" — the band from the flow root down to X's family; same walk `reset_downstream` uses), `flow.dependencies(X)` the forward one (X's upstream cone). `X` may be a **class, family string, or instance** — a class/string is *not* instantiated, so it works for fanned-out / DAG-internal families that `get_task()` can't build (that is the point — `find_deps` takes a family string precisely to sidestep `MissingParameterException` on a mid-DAG task). `paths=True` returns the ordered `root→X` routes (a list of task lists) instead of the deduped set — so you can see *how many* distinct routes reach X, not merely that it's reached. Reach for these instead of grep or a hand-rolled `requires()` walk: the module-level primitives are `core.find_deps` (set) / `core.find_paths` (ordered, re-exported as `oryxflow.find_paths`), both memoized (polynomial for the set; `find_paths` materializes every distinct path by nature). See `docs/todo/20260803-engine-dependency-queries.md`.

**Unused-input lint** — a declared dependency whose data `run()` loads and never reads is invisible to every dependency query (the edge is *real*; only the data is dead) yet still forces its whole upstream band on every cold build. `flow.check_inputs()` (static AST, `oryxflow/inputcheck.py`) reports these; `preview()` surfaces them automatically in an `UNUSED INPUTS` block (deduped per family). **`run()`/`build()` deliberately do NOT lint** — the execution path stays free of advisory work — so a finding does **not** land in `RunResult.warnings`, emit an event, or raise `UnusedInputWarning` on the automatic path; call `check_inputs()` explicitly for CI. (The AST parse is memoized per class in `inputcheck`, so it was a one-time-per-process cost even in `run()`; it was still pulled out on the principle that `run()` carries no advisory work — see the divergence note in the plan.) Three verdicts, never two — `unused` / `clean` / `unanalyzed` (a shape it can't prove is `unanalyzed`, never silently `clean`); **outer** unpack elements are dependencies, **inner** are that dep's `persists` (an inner `_` is normal, a top-level `_` is a finding). Suppress a deliberate one with `# oryxflow: input-unused`. `dependents()` and `check_inputs()` answer *different* questions — the first "does anything depend on X", the second "is a declared edge actually used" — and neither substitutes for the other. See `docs/todo/20260803-engine-unused-input-lint.md`.
...
**Fan-out group selectors.** When a fan-out is stacked with other deps, `inputLoad(flatten=False)` nests the branches under their group name (`{'input': df, 'RegionNarrative': {'north': ..., ...}}`) so `run()` doesn't hand-roll "pop the keys I know, iterate the rest"; `inputLoad(task='<group>')` / `inputLoadConcat(task='<group>')` select just the branches; `inputLoadConcat(flatten=False)` returns one frame per group. Grouping is a **return-value shape only** — `requires()` and `input()` stay flat, because a nested `input()` (`{'narrative': {'north': target}}`) is indistinguishable from a multi-`persists` dep, which is already `{persist: target}`. `inputLoadConcat()` with no selector **warns** when the spec holds both a fan-out and a shared dep (it would row-stack the shared one in with the branches — a union frame). Rejected and not to be added without revisiting `docs/todo/20260729-engine-fanout-requires-each.md`: `inputLoad(concat=True)` / `fanout=True` and a fourth `inputLoadEach()` accessor.
