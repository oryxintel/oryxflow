...
class TestWorkflowMulti:
    ...
    def test_preview_single_flow(self):
        flow2 = oryxflow.WorkflowMulti(params = {'experiment1': {'do_preprocess': False}, 'experiment2': {'do_preprocess': True}},
                                  task=TaskTrain)
        # Reset
    ...
    def test_multi_get_task(self):
        params2 = {'param1': 2}
        params = {'param1': 1}
        flow2 = oryxflow.WorkflowMulti(params = {1: params, 2: params2}, task=Task1)
    ...
    def test_multi_params_generator(self):
        ...
        flow2 = oryxflow.WorkflowMulti(params = params, task=TaskTrain)
    ...
    def test_multi_get_task_with_list_params(self):
        params2 = {'param1': 2}
        params = {'param1': 1}
        flow2 = oryxflow.WorkflowMulti(params = [params, params2], task=Task1)
    ...
    def test_outputLoadConcat(self):
        flow = oryxflow.WorkflowMulti(ConcatCountry, params={'country': ['US', 'UK']})
        flow.run()
        df = flow.outputLoadConcat(ConcatCountry)
        assert set(df['country']) == {'US', 'UK'}
        assert len(df) == 6  # 3 rows per flow x 2 flows

    def test_runIterConcat(self):
        df = oryxflow.runIterConcat(ConcatCountry, params={'country': ['US', 'UK']})
        assert set(df['country']) == {'US', 'UK'}
        assert len(df) == 6


# Native aggregator concat task for outputLoadConcat/runIterConcat tests
class ConcatCountry(oryxflow.tasks.TaskCachePandas):
...
class TestWorkflowReset:
    ...
    def test_reset_downstream_multi(self):
        ...
        flow_multi = oryxflow.WorkflowMulti(params = {'experiment1': {'do_preprocess': True}, 'experiment2': {'do_preprocess': True}},
                                  task=PathTask3)
        flow_multi.run()
        assert PathTask1().complete(cascade=False) and PathTask3(do_preprocess=True).complete(cascade=False)
        # reset_downstream(PathTask1): resets the band PathTask1..PathTask3 (endpoint defaults to
        # the flow's default task PathTask3) across every flow
        flow_multi.reset_downstream(PathTask1)
        assert not PathTask1().complete(cascade=False)
        assert not PathTask3(do_preprocess=True).complete(cascade=False)

    def test_reset_upstream_default_anchor(self):
        # flow.reset_upstream(only=Family) with NO anchor -> uses the flow's default task, and
        # resets that family across every flow while preserving the leaf.
        ran = []
        class ALeaf(oryxflow.tasks.TaskCachePandas):
            grp = oryxflow.Parameter(); sub = oryxflow.Parameter()
            def run(self): ran.append(('ALeaf', self.grp, self.sub)); self.save(pd.DataFrame({'v': range(2)}))
        class AMid(oryxflow.tasks.TaskCachePandas):
            grp = oryxflow.Parameter()
            def requires(self): return {s: ALeaf(grp=self.grp, sub=s) for s in ['a', 'b']}
            def run(self): ran.append(('AMid', self.grp)); self.save(self.inputLoadConcat())

        flow = oryxflow.WorkflowMulti(AMid, params={'grp': ['x', 'y']})
        flow.run(); ran.clear()
        flow.reset_upstream(only=AMid)            # no anchor -> defaults to AMid (flow default task)
        flow.run()
        assert not any(r[0] == 'ALeaf' for r in ran)                      # leaf preserved
        assert sorted(r for r in ran if r[0] == 'AMid') == [('AMid', 'x'), ('AMid', 'y')]

    def test_reset_downstream_single_flow(self):
        # reset_downstream(task, flow=one) resets only that flow's downstream band
        class SMid(oryxflow.tasks.TaskCachePandas):
            region = oryxflow.Parameter(); grp = oryxflow.Parameter()
            def run(self): self.save(pd.DataFrame({'v': range(2)}))
        class STop(oryxflow.tasks.TaskCachePandas):
            region = oryxflow.Parameter()
            def requires(self): return {g: SMid(region=self.region, grp=g) for g in ['a', 'b']}
            def run(self): self.save(self.inputLoadConcat())

        flow = oryxflow.WorkflowMulti(STop, params={'x': {'region': 'x'}, 'y': {'region': 'y'}})
        flow.run()
        flow.reset_downstream(SMid, flow='x')
        assert not STop(region='x').complete(cascade=False)              # flow x band reset
        assert not SMid(region='x', grp='a').complete(cascade=False)
        assert STop(region='y').complete(cascade=False)                  # flow y untouched
        assert SMid(region='y', grp='a').complete(cascade=False)
...
class TestWorkflowOutput:
    ...
    def test_workflow_multi_outputPath(self):
        flow_multi = oryxflow.WorkflowMulti(params = {'experiment1': {'do_preprocess': False}, 'experiment2': {'do_preprocess': True}},
                                  task=PathTask3)
        flow_multi.run()
        ...
        assert path_1 == {
            'experiment1': Path('data/PathTask1/PathTask1__99914b932b-data.cache'),
            'experiment2': Path('data/PathTask1/PathTask1__99914b932b-data.cache')
        }
        assert path_2 == {
            'experiment1': Path('data/data2_changed/PathTask2/PathTask2__99914b932b-data.parquet'),
            'experiment2': Path('data/data2_changed/PathTask2/PathTask2__99914b932b-data.parquet')
        }
        ...
        assert path_1 == {
            'experiment1': 
                {
                    'xx': Path('data/PathTaskMultiOutput/PathTaskMultiOutput__99914b932b-xx.parquet'),
                    'yy': Path('data/PathTaskMultiOutput/PathTaskMultiOutput__99914b932b-yy.parquet')
                },
            'experiment2':
                {
                    'xx': Path('data/PathTaskMultiOutput/PathTaskMultiOutput__99914b932b-xx.parquet'),
    ...
    def test_workflow_set_path_and_env(self):
        # Execute task including all its dependencies
        flow = oryxflow.WorkflowMulti(params = {'experiment1': {'do_preprocess': False}, 'experiment2': {'do_preprocess': True}},
        ...
        assert path_2 == {
            'experiment1': Path('data/data2/wfm_change/env=prod/PathTask2/PathTask2__99914b932b-data.parquet'),
            'experiment2': Path('data/data2/wfm_change/env=prod/PathTask2/PathTask2__99914b932b-data.parquet')
        }
        assert path_3 == {
            'experiment1': Path('data/data2/wfm_change/env=prod/PathTask3/PathTask3__99914b932b-data.parquet'),
            'experiment2': Path('data/data2/wfm_change/env=prod/PathTask3/PathTask3__99914b932b-data.parquet')
        }
        flow.reset()
